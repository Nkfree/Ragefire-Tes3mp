...
function lootSharing.CellMessage(pid)
    if utils.PlayerPartyCheck(pid) then
    	local lootShuffle = partySystem.utils.GetPlayerParty(pid)["lootShuffle"]
    	local goldSplitting = partySystem.utils.GetPlayerParty(pid)["goldSplitting"]
    	if (lootShuffle or goldSplitting) and partySystem.lootSharing.IsMerchantCell(pid) then        	
    		local cellMessage = ""
    		if lootShuffle and goldSplitting then cellMessage = "Loot shuffle and gold splitting are disallowed in this area." 
    		elseif lootShuffle then cellMessage = "Loot shuffle is disallowed in this area." 
    		elseif goldSplitting then cellMessage = "Gold splitting is disallowed in this area." end
        	tes3mp.MessageBox(pid, -1, cellMessage)
        end
	end
end
...
function lootSharing.SplitGold(pid, goldAmount)
	...
	local elligiblePidArray = {}                                     		 --We have to loop through dynamic members to
