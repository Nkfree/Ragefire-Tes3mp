...
function utils.PartyExists(partyName)
	...
	else tes3mp.LogMessage(enumerations.log.ERROR, "Nil value passed to utils.PartyExists!") end
end
...
function utils.DynamicPartyExists(partyName)
	...
	else tes3mp.LogMessage(enumerations.log.ERROR, "Nil value passed to utils.DynamicPartyExists!")	end
end
...
function utils.IsPlayerDynamicPartyMember(partyName,playerName)
	...
	else tes3mp.LogMessage(enumerations.log.ERROR, "Nil value(s) passed to utils.IsPlayerDynamicPartyMember!") end
end
...
function utils.IsPlayerInAnyParty(pid)
	if Players[pid].data.customVariables.partyName then return Players[pid].data.customVariables.partyName ~= ""
	else
		Players[pid].data.customVariables.partyName = ""
