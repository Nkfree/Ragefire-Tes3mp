...
startupData = {}
startupScripts.Initialize = function(eventStatus)
...
startupScripts.InitializeCustomVariables = function(pid)
...
startupScripts.OnLogin = function(eventStatus, pid)
...
startupScripts.LoadExpulsionTimer = function(pid)
...
startupScripts.CheckGuildExplusion = function(pid)
    local expulsion = false
...
startupScripts.DisableCharGen = function(pid)
...
startupScripts.LoadProgressedWorld = function(pid)
    ...
    for index, value in ipairs (startupJournal) do
        ...
        for i2 = 1, i-1 do
            if journal[indexArray[i2]].index == startupIndex then
                ...
                if startupEntry.scriptName ~= nil then
                    if startupEntry.timerName ~= nil then
                        local daysPassed = WorldInstance.data.time.daysPassed - journal[indexArray[i2]].timestamp.daysPassed
                        if daysPassed > 0 then
                            logicHandler.RunConsoleCommandOnPlayer(pid, "set " .. startupEntry.scriptName .. "." .. startupEntry.timerName .. " to " .. daysPassed)
                        end
                    end
                end
            end
        end
        -- over-ride the outcome array if we're working with estate quest
        if startupQuest == "co_estate" then
            if startupIndex == 40 then
                local newState = 30+estatePosition
                commandArray[1] = "set colonyState to " .. newState
            end
            if startupIndex == 50 then
                local newState = 33+estatePosition
                commandArray[1] = "set colonyState to " .. newState
            end
        end
        for j2 = 1, j-1 do
...
startupScripts.LoadVampirism = function(pid)
...
startupScripts.RemoveBoundItems = function(pid)
...
startupScripts.OnCellChange = function(eventStatus, pid)
...
startupScripts.RunStartup = function(eventStatus, pid)
...
customEventHooks.registerHandler("OnServerInit", startupScripts.Initialize)
customEventHooks.registerHandler("OnPlayerCellChange", startupScripts.OnCellChange)
customEventHooks.registerHandler("OnPlayerConnect", startupScripts.RunStartup)
customEventHooks.registerHandler("OnPlayerFinishLogin", startupScripts.OnLogin)
customEventHooks.registerHandler("OnPlayerEndCharGen", startupScripts.OnLogin)
