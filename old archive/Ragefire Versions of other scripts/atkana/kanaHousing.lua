...
local function getHouseInfoLong(houseName, toggleMeanings) --Used in GUI labels
	...
	text = color.Gold .. text .. color.White .. "House Value: " .. color.Gold .. hdata.price .. color.White .. " gold\n\n Currently owned by: " ..
				color.DarkGray .. owner .. "\n\n" .. color.White .. ""
	
	
	local hasOwned, hasAccess, hasResets
	text = text .. "Contains following cells: "
	for cellName, v in pairs(hdata.cells) do
		local cdata = housingData.cells[cellName]
		local addText = " "
		addText = color.Gold .. addText .. cellName
		...
		addText = addText .. "\n\n"
		text = text .. addText .. color.White .. "Would you like to " .. color.Gold .. "purchase " .. color.White .. "this house?"
	end
...
showPlayerSettingsMain = function(pid)
	...
	message = message .. "\n"
	message = message .. "Use 'Select Owned House' to select the house whose settings you want to edit. To add a Co-owner, use 'Add Co-owner', or remove one with 'Remove Co-owner'. Co-Owners can place items in the house too. Use 'Sell House' if you want to sell it."
	
	return tes3mp.CustomMessageBox(pid, config.PlayerSettingGUI, message, "Select Owned House;Add Co-owner;Remove Co-owner;Sell House;Close")
end
...
local function onHouseInfoBuy(pid)
	...
	if hdata then
		...
		elseif getHouseOwnerName(hdata.name) == getName(pid) then
			...
			return tes3mp.SendMessage(pid, color.LightGreen .. "Congratulations! You are now the proud owner of " .. color.RoyalBlue .. hdata.name .. "." .. color.White .. "\nUse the " ..
				color.Gold .. "/house " .. color.White .. "command to manage your house's settings.\n", false)
		end
	else
...
Methods.OnGUIAction = function(pid, idGui, data)
	...
	elseif idGui == config.PlayerSettingGUI then --Player Setting Main
		...
		elseif tonumber(data) == 2 then --Remove Co-owner
			...
			return true
		elseif tonumber(data) == 3 then --Sell House
		...
		if housingData.cells[currentCell] and housingData.cells[currentCell].house ~= nil then
			...
			if enterReason == "unowned" then --A player has entered a house without an owner
				if lastEnteredHouse[getName(pid)] ~= hdata.name then --They've just entered the house
					msg(pid, color.Gold .. "------------------------------------------------\n" ..
								color.White .. "Currently viewing: " .. color.Gold .. hdata.name .. color.White .. "\nCost: " .. color.Gold .. hdata.price .. color.White .. " gold\n" ..
									color.White .. "Type " .. color.Gold .. "/buy " .. color.White .. "to purchase it.\n" ..
										color.Gold .. "------------------------------------------------")
				end
			...
			elseif enterReason == "unlocked" then				
				if lastEnteredHouse[getName(pid)] ~= hdata.name then --They've just entered the house
					if isOwner(getName(pid), hdata.name) or isCoOwner(getName(pid), hdata.name) then
						...
						Players[pid].data.isInOwnedHouse = true
