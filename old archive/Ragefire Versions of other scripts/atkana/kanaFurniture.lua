...
local function getPlayerGold(pid)
	local goldLoc = inventoryHelper.getItemIndex(Players[pid].data.inventory, "tie_goldendrake", -1)
...
local function addGold(pid, amount)
	...
	if goldLoc then
		...
		table.insert(Players[pid].data.inventory, {refId = "tie_goldendrake", count = amount, charge = -1})
	end
