...
- Version for Ragefire
- allows for server design instead of worldmining
- dont use miscellaneous.json
- dont use the french furn
- (still watch out for french items)
...
local config = {}
config.DontRemoveFromInventory = true -- if true wont remove items from inventory when they are placed
config.noCost = true -- if true no one will have to pay
...
config.PromptId = 91369

local message = {}
message.PromptId = "Enter RefNum to Delete"
...
message.OnBuildOption = "Creatures;French Furn;Furn2;Dungeon;Rocks;Plants;Ext;Statics;Doors;Back"
message.WhereDoor = "where should this door go?"
message.WelcomeFurn = color.Green.."Welcome to the Worlddesigner.\n\n"..color.Yellow.."Construction "..color.White.."to buy your objects you need and place doors\n\n"..color.Yellow.."Inventory "..color.White.."to see the things you can place\n\n"..color.Yellow.."Stock "..color.White.."to see all the things you placed\n\n"
message.MainChoice = "Inventory;Stock;Construction;Material;Back;Delete by Activate;Enter RefNum to Delete"
...
WorldMining.OnGUIAction = function(eventStatus, pid, idGui, data)
	...
	if idGui == config.MainGUI then -- Main
		...
		elseif tonumber(data) == 5 then
			WorldMining.DeleteMode(pid)
			return true
		elseif tonumber(data) == 6 then
				tes3mp.InputDialog(pid, config.PromptId, message.PromptId,"")
			return true
		end
	elseif idGui == config.PromptId then
		if data ~= nil and data ~= "" and tonumber(data) then
			WorldMining.DeleteRefNum(pid, tonumber(data))
			return true
		end
...
addFurnitureItem = function(pname, refId, count, save)
	...
	if config.DontRemoveFromInventory == false then
...
refNumDeletionsByCell = jsonInterface.load("refNumDeletionsByCell.json") -- check if this overrides. it doesnt


WorldMining.FixCell = function(eventStatus, pid)
...
WorldMining.DeleteMode = function(pid)
	tes3mp.MessageBox(pid, -1, "activate an object to add it to refNumDeletions")
	Players[pid].DeleteMode = true
end

WorldMining.DeleteRefNum = function(pid, ObjectRefNum)

objectUniqueIndex = ObjectRefNum.."-0"
				...
				logicHandler.DeleteObject(pid, cellDescription, objectUniqueIndex, true)

                tes3mp.SendMessage(pid, "Deleted refNum: " .. ObjectRefNum .. ", but guessed the uniqueindex.\n Use /furn to activate deletemode again\n", false)
end

WorldMining.OnDeleteActivate = function(eventStatus, pid, cellDescription, objects, players)
isValid = true
	if objects[1] == nil then return customEventHooks.makeEventStatus(isValid, isValid) end
		   if Players[pid].DeleteMode ~= nil and Players[pid].DeleteMode == true then -- disables all activations and adds them to refnumdeletions
                isValid = false
                objectRefId = objects[1].refId
					objectUniqueIndex = objects[1].uniqueIndex
					local splitIndex = objectUniqueIndex:split("-")
                ...
                tes3mp.SendMessage(pid, "Deleted refNum: " .. ObjectRefNum .. "\n Use /furn to activate deletemode again\n", false)
				Players[pid].DeleteMode = false
				return customEventHooks.makeEventStatus(isValid, isValid)
			end
end

WorldMining.OnRefCommand = function(pid, cmd)
if cmd[2] ~= nil and (cmd[2] == "place" or cmd[2] == "spawn") and cmd[3] ~= nil then
	local choice = {}
	choice.need = cmd[2]
	choice.refId = cmd[3]
	local curCell = tes3mp.GetCell(pid)
...
customEventHooks.registerHandler("OnPlayerCellChange", WorldMining.FixCell)
customEventHooks.registerValidator("OnObjectActivate", WorldMining.OnDeleteActivate)
...
customCommandHooks.registerCommand("ref", WorldMining.OnRefCommand)
